    ...
    return SafeArea(
      child: ListView(
        ...
        children: [
          ...
          const Row(children: [
            Expanded(child: _StatCard(value: '0', label: 'sesiones')),
            SizedBox(width: 10),
            Expanded(child: _StatCard(value: '0 min', label: 'entrenados')),
            SizedBox(width: 10),
            Expanded(child: _StatCard(value: '0', label: 'racha')),
          ]),
          const SizedBox(height: 26),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6), decoration: BoxDecoration(color: accent.withValues(alpha: .14), borderRadius: BorderRadius.circular(10)), child: Text('PARA HOY', style: TextStyle(color: accent, fontSize: 11, fontWeight: FontWeight.w900))),
          const Spacer(),
          const Icon(Icons.auto_awesome_rounded, color: Colors.white54),
        ]),
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: ListView(
        padding: const EdgeInsets.all(20),
        children: [
          const _TopBar(title: 'ENTRENAR'),
          const SizedBox(height: 26),
          const Text('Elige cómo quieres entrenar', style: TextStyle(fontSize: 28, fontWeight: FontWeight.w900)),
          const SizedBox(height: 18),
          InkWell(onTap: onStart, borderRadius: BorderRadius.circular(18), child: const _InfoCard(icon: Icons.auto_awesome_rounded, title: 'Entrenamiento para ti', text: 'Generado según tu perfil actual. Toca para empezar.')),
          const SizedBox(height: 12),
          const _InfoCard(icon: Icons.timer_outlined, title: 'Sesiones rápidas', text: 'Entrenamientos para cuando tienes poco tiempo.'),
          const SizedBox(height: 12),
          const _InfoCard(icon: Icons.grid_view_rounded, title: 'Por zona muscular', text: 'Brazos, pecho, abdominales, piernas, espalda o cuerpo completo.'),
          const SizedBox(height: 12),
          SizedBox(
            width: double.infinity,
            height: 52,
            child: OutlinedButton.icon(
              onPressed: () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => WorkoutPage(profile: profile))),
...
class _ProgressTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) => const SafeArea(
    child: SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        _TopBar(title: 'PROGRESO'),
        SizedBox(height: 26),
        Text('Tu progreso', style: TextStyle(fontSize: 30, fontWeight: FontWeight.w900)),
        SizedBox(height: 18),
        _InfoCard(icon: Icons.insights_rounded, title: 'Aún estamos empezando', text: 'Cuando completes entrenamientos aparecerán aquí tus estadísticas, racha y evolución.'),
      ]),
    ),
  );
}
  ...
  Widget build(BuildContext context) => SafeArea(
    child: ListView(
      ...
      children: [
        ...
        const SizedBox(height: 26),
        const Text('Tu perfil', style: TextStyle(fontSize: 30, fontWeight: FontWeight.w900)),
        const SizedBox(height: 18),
        _InfoCard(icon: Icons.flag_outlined, title: 'Objetivos', text: profile.goals.join(', ')),
        const SizedBox(height: 10),
        _InfoCard(icon: Icons.speed_rounded, title: 'Nivel', text: profile.level?.name ?? '-'),
        const SizedBox(height: 10),
        _InfoCard(icon: Icons.schedule_rounded, title: 'Tiempo', text: profile.duration?.name ?? '-'),
        const SizedBox(height: 10),
        _InfoCard(icon: Icons.fitness_center_rounded, title: 'Equipamiento', text: profile.equipment.map((e) => e.name).join(', ')),
      ],
    ),
  );
}
...
class _TopBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final accent = Theme.of(context).colorScheme.primary;
    return Row(children: [
      Container(width: 42, height: 42, decoration: BoxDecoration(color: accent.withValues(alpha: .14), borderRadius: BorderRadius.circular(13)), child: Icon(Icons.bolt_rounded, color: accent)),
      const SizedBox(width: 12),
      Text(title, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w900, letterSpacing: 2)),
      const Spacer(),
...
class _StatCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    decoration: BoxDecoration(color: const Color(0xFF15181D), borderRadius: BorderRadius.circular(16), border: Border.all(color: Colors.white10)),
    child: Column(children: [Text(value, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w900)), const SizedBox(height: 3), Text(label, style: const TextStyle(color: Colors.white54, fontSize: 11))]),
  );
}
